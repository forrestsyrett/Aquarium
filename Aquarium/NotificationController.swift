...
//        let currentWeekDay = myComponents.weekday
        ...
        feedNotification.userInfo = ["key": "\(feeding.info.animalName)"]
        ...
        // get the date on or after today that matches the weekday
        // set the time from the feeding on that date minus 15 minutes
        // schedule the notification using the feeding.info
        // use the animal name and the time string to construct the notification title
    

    // Code above replaces code below.
    // all of the code below can be removed.
